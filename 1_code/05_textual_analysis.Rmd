...
In this sesssion will we use the SEC EDGAR 10-K text files and the annual reports from www.AnnualReport.com we downloaded in on day 1 in session 1 and session 2. The purpose of this session is to show the procedure how to extract information from unstructured text.

Throughout this script we will use custom functions to download and transform data. All the script specific functions are stored in this folder: **1_code/00_functions/f-f-textual_analysis.R**
...
First we read in all the raw files stored in the session specific folders for EDGAR and www.AnnualReport.com
...
```{r}
if (!file.exists(lst_paths$path_frq_1gram)) {
  frq_1gram <- prep_top_n_by(tab_1gram, tab_data, country, 20) %>%
  display_top_n_by(country)
  write_rds(frq_1gram, lst_paths$path_frq_1gram)
} else {
  frq_1gram <- read_rds(lst_paths$path_frq_1gram)
}
frq_1gram
```

```{r}
if (!file.exists(lst_paths$path_frq_2gram)) {
  frq_2gram <- prep_top_n_by(tab_2gram, tab_data, country, 20) %>%
  display_top_n_by(country)
  write_rds(frq_2gram, lst_paths$path_frq_2gram)
} else {
  frq_2gram <- read_rds(lst_paths$path_frq_2gram)
}
frq_2gram
```
...
if (!file.exists(lst_paths$path_com_1gram)) {
  ...
  write_rds(com_1gram, lst_paths$path_com_1gram)
} else {
...
if (!file.exists(lst_paths$path_com_2gram)) {
  ...
  write_rds(com_2gram, lst_paths$path_com_2gram)
} else {
...
if (!file.exists(lst_paths$path_lda)) {
  lda <- LDA(cast_dtm(tab_1gram, doc_id, word, n), k = 10, control = list(seed = 1234))
...
```{r}
chapters_lda <- LDA(cast_dtm(tab_1gram, doc_id, word, n), k = 10, control = list(seed = 1234))
chapters_lda

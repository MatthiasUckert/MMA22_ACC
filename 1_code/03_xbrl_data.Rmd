...
We use a custom function: **xbrl_process_elements()** to extract elements from the XBRL tagged .xml files. This function is heavily based on the XBRL library. Nonetheless, we implemented some changes to correct for a known bug in the source code, streamline the download process and fit it into our normal workflow.
